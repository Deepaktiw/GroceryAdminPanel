...
export class ManageSubCategoryComponent implements OnInit {
  ...
  Columns = ['No','categoryImage','categoryName',  'Action'];
  ...
  addGroup(dm) {
    ...
    formData.append("groupCode", this.dm.groupName);
    formData.append("image", this.dm.image);
    this.http.post(`${environment.providerDomain}/api/v1/group/subgroup/`, formData).subscribe(res => {
  ...
  updateGroup(vm) {
    this.spinner.show();
    this.httpCall.put(`${environment.providerDomain}/api/v1/group/subgroup/`, this.vm).subscribe(res => {
      if (res['code'] == 200 || res['code'] == 201) {
  ...
  deleteCategory(code) {
    ...
    Swal.fire({
      ...
      if (result.value) {
        this.httpCall.delete(`${environment.providerDomain}/api/v1/group/subgroup/`, { body }).subscribe(res => {
          console.log(res);
          this.spinner.hide();
          if (res['code'] == 200) {
            this.notification.showNotification('success', res['message']);
  ...
  editCategory(element) {
    try {
      if (element != null) {
        ...
        this.vm.groupName = element.groupName;
        this.vm.image = element.image;
      } else {
        console.log("else");
      }
    } catch (err) {
